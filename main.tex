...
\large
\vspace{0.5 cm}
Matching is equivalent to nested \mintinline{swift}{xs.map(x => ys.map(y => ...))}, apart from the uniqueness and symmetry criteria.

\vspace{1 cm}
\uncover<2->{It relies on tracking the {\it identity} of each object, including composite identity \\ (e.g.\ Z in terms of its constituent leptons).}

\vspace{1 cm}
\uncover<3->{Gordon Watts introduced the idea of considering each physics event as a (tiny) relational database. Thus, each input table (collection of particles) has a different surrogate index. Derived quantities propagate those indexes, and joining tables creates composite indexes.}
\end{frame}
